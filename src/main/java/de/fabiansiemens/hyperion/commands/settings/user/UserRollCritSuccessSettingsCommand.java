...
@Slf4j
@Subcommand(dataFile = "settings/user/CritSuccess.json", parent = UserSettingsCommand.class, group = "roll")
public class UserRollCritSuccessSettingsCommand extends SlashCommandBase {
	...
	public UserRollCritSuccessSettingsCommand(
				UserService userService, 
				ApplicationContext context,
				ImageCompressionService imageCompressionService
			) throws IOException, IllegalArgumentException {
		super(UserRollCritSuccessSettingsCommand.class, context);
	...
	public void performSlashCommand(SlashCommandInteractionEvent event) throws UiParseException {
		...
		if(!message.isPresent() && ! attachment.isPresent()) {
			if(settings.getCritSuccessFile() == null) {
				event.getHook().sendMessage(les.getLocalizedExpression("success.settings.overview", event.getGuild())
						+ "\n> **message:** " + (settings.getCritSuccessMessage() == null ? "--" : settings.getCritSuccessMessage()) 
			...
			event.getHook().sendMessage(les.getLocalizedExpression("success.settings.overview", event.getGuild())
					+ "\n> **message:** " + (settings.getCritSuccessMessage() == null ? "--" : settings.getCritSuccessMessage()) 
					+ "\n> **image:**")
			.addFiles(FileUpload.fromData(settings.getCritSuccessFile().getData(), "crit-success-image." + settings.getCritSuccessFile().getExtension()))
			.setEphemeral(true)
			.queue();
			return;
		}
		...
		if(message.isPresent()) {
			...
			settings.setCritSuccessMessage(messageContent);
		}
		...
		if(attachment.isPresent()) {
			...
			try {
				...
				settings.setCritSuccessFile(new FileEntity(data, attachmentContent.getFileExtension()));
